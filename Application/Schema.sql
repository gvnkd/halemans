...
-- Local metric cache (milestone 13): on-demand per-viewed-chart storage of
-- fetched series in the generalized [MetricSeries] shape. zabbix series_key =
-- "z:<itemid>", grafana = "g:<datasourceUid>:<expr hash>". Hourly buckets;
-- closed buckets are immutable, only the bucket containing now is re-fetched
-- (freshen window). Retention is opportunistic (delete-on-store, same
-- source_id), default 7 days via sources.config.metrics.cacheRetentionDays.
