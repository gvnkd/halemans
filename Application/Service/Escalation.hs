...
import Application.Pipeline.Escalation (stepsFromJSON, stepDeadline)
...
createTracker alert rule policyId = do
    existing <- query @EscalationTracker
    ...
    case existing of
        ...
        Nothing -> do
            ...
            case stepsFromJSON policy.steps of
                ...
                (firstStep:_) -> do
                    now <- getCurrentTime
                    tracker <- newRecord @EscalationTracker
...
cancelTrackersFor alertId = do
    now <- getCurrentTime
    active <- query @EscalationTracker
        |> filterWhere (#alertId, alertId)
        |> filterWhere (#status, "active" :: Text)
        |> fetch
    forM_ active \tracker -> void $ tracker
...
restartTrackersFor alertId = do
    cancelled <- query @EscalationTracker
        |> filterWhere (#alertId, alertId)
