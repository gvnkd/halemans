...
import IHP.Prelude
import IHP.ModelSupport (ModelContext, newRecord, createRecord, updateRecord)
...
currentToolCacheTtl = do
    ...
    pure case head rows of
        Just row | not row.enabled -> Nothing
                ...
                _ -> do
                    ...
                    unless (isFailureText response) do
                        _ <- case cached of
                            ...
                            Nothing -> createRecord (newRecord @LlmToolCache
                                |> set #tool tool
