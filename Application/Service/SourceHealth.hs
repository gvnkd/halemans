...
--
-- The raised alerts carry a real subject (env from the source row, service =
-- source name, host = the halemans host generating the alert) so grouping,
-- blackout matching and notification routing see them like any other alert
-- instead of dashes.

healthFingerprint :: Id Source -> Text
healthFingerprint sourceId = "halemans:source-health:" <> tshow sourceId

-- | Host running halemans. Internal alerts (source health, reconcile) are
-- generated by halemans itself, so host points at us rather than at the
-- monitored target; the source identity lives in the title/labels/service.
halemansHostName :: IO Text
halemansHostName = cs . nodeName <$> getSystemID
