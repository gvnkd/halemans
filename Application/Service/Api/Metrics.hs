...
collectMetrics = do
    ...
    pure $
        Text.concat
            ...
            , renderFamily
                "halemans_source_consecutive_failures"
                [ sample
                    "halemans_source_consecutive_failures"
                    [("source", source.name)]
                    (tshow source.consecutiveFailures)
                | source <- sources
                ]
            , renderFamily
                "halemans_source_healthy"
                [ sample
                    "halemans_source_healthy"
                    [("source", source.name)]
                    (if source.consecutiveFailures == 0 then "1" else "0")
                | source <- sources
                ]
            , renderFamily
                "halemans_job_runs_total"
                [ sample
                    "halemans_job_runs_total"
            ...
            , renderFamily "halemans_llm_tokens_today" $
                concat
                    [ [ sample
                            "halemans_llm_tokens_today"
                            [("provider", get #provider row), ("direction", "in")]
                            (tshow (get #tokens_in row))
                      , sample
                            "halemans_llm_tokens_today"
                            [("provider", get #provider row), ("direction", "out")]
                            (tshow (get #tokens_out row))
                      ]
                    | row <- llmCounters
                    ]
            , renderFamily
                "halemans_ws_connections"
