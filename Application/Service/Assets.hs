...
import Application.Service.Assets.Errors (AssetsError (..), classifyResponse, describeError)
...
listSchemas client =
    decodeList ["objectschemas", "values", "objects"]
        <$> getJson client Nothing "/objectschema/list" []

listTypesFlat :: AssetsClient -> Int64 -> IO (Either AssetsError [ObjectType])
listTypesFlat client schemaId =
    decodeList ["objecttypes", "values", "objects"]
        <$> getJson
            client
            Nothing
            ("/objectschema/" <> tshow schemaId <> "/objecttypes/flat")
...
searchObjects client aql page resultPerPage =
    decodeOne
        <$> getJson
            client
            Nothing
...
connectedTickets client objectId =
    decodeList ["tickets", "values"]
