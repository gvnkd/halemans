...
instance View IndexView where
    html IndexView{..} =
        ...
        {inlinePostFormHtml (pathTo TestLlmConnectionAction) (tr "Test connection") "btn btn-sm btn-outline-primary" (Just "test-llm") False}
        ...
        <form method="POST" action={UpdateAutoAnalyzeAction} class="maxw-700" data-testid="auto-analyze-form">
            ...
            <button type="submit" class="btn btn-sm btn-primary" data-testid="auto-analyze-submit">{tr "Save"}</button>
        </form>
        ...
        <form method="POST" action={UpdateToolCacheAction} class="maxw-700" data-testid="tool-cache-form">
            ...
            <button type="submit" class="btn btn-sm btn-primary" data-testid="tool-cache-submit">{tr "Save"}</button>
        </form>
    ...
    |]
      where
        newProviderButton = [hsx|<a href={NewLlmProviderAction} class="btn btn-sm btn-primary" data-testid="new-llm-provider">{tr "New provider"}</a>|]
        ttlValue :: Text
        ttlValue = maybe "300" tshow toolCacheTtl
        newRoleButton = [hsx|<a href={NewLlmRoleAction} class="btn btn-sm btn-primary" data-testid="new-llm-role">{tr "New role"}</a>|]
        templateActions =
            [hsx|
                <div>
                    <a href={LlmQueueAction} class="btn btn-sm btn-outline-secondary" data-testid="llm-queue-link">{tr "Queue"}</a>
                    <a href={NewLlmTemplateAction} class="btn btn-sm btn-primary" data-testid="new-llm-template">{tr "New template"}</a>
                </div>
            |]
...
providerRowHtml provider =
    ...
    <tr data-testid="llm-provider">
        ...
        <td>{enabledBadge}</td>
        <td>
            <a href={EditLlmProviderAction providerId} class="btn btn-sm btn-outline-secondary" data-testid="llm-provider-edit">{tr "Edit"}</a>
            {toggleForm}
            {deleteForm}
        </td>
    </tr>
|]
  where
    ...
    toggleForm =
        if provider.enabled
            then inlinePostFormHtml (pathTo (DisableLlmProviderAction providerId)) (tr "Disable") "btn btn-sm btn-outline-warning" (Just "llm-provider-disable") False
            else inlinePostFormHtml (pathTo (EnableLlmProviderAction providerId)) (tr "Enable") "btn btn-sm btn-outline-primary" (Just "llm-provider-enable") False
    deleteForm = inlinePostFormHtml (pathTo (DeleteLlmProviderAction providerId)) (tr "Delete") "btn btn-sm btn-outline-danger" (Just "llm-provider-delete") False
...
roleRowHtml role =
    ...
    <tr data-testid="llm-role">
        ...
        <td>{defaultBadge}</td>
        <td>
            <a href={EditLlmRoleAction roleId} class="btn btn-sm btn-outline-secondary" data-testid="llm-role-edit">{tr "Edit"}</a>
            {toggleForm}
            {defaultForm}
            {deleteForm}
        </td>
  ...
  where
    ...
    defaultForm =
        if role.isDefault || not role.enabled
            then mempty
            else inlinePostFormHtml (pathTo (SetDefaultLlmRoleAction roleId)) (tr "Set default") "btn btn-sm btn-outline-primary" (Just "llm-role-set-default") False
    deleteForm =
        if role.isDefault
            then mempty
            else inlinePostFormHtml (pathTo (DeleteLlmRoleAction roleId)) (tr "Delete") "btn btn-sm btn-outline-danger" (Just "llm-role-delete") True
...
templateRowHtml row =
    ...
    <tr data-testid="llm-template">
        ...
        <td>
            <a href={EditLlmTemplateAction row.templateId} class="btn btn-sm btn-outline-secondary" data-testid="llm-template-edit">{tr "Edit (new version)"}</a>
            {activateForm}
            {deleteForm}
        </td>
    </tr>
|]
  where
    ...
    activateForm =
        if row.active
            then mempty
            else inlinePostFormHtml (pathTo (ActivateLlmTemplateAction row.templateId)) (tr "Activate") "btn btn-sm btn-outline-primary" (Just "llm-template-activate") False
    deleteForm =
        if row.active
            then mempty
            else inlinePostFormHtml (pathTo (DeleteLlmTemplateAction row.templateId)) (tr "Delete") "btn btn-sm btn-outline-danger" (Just "llm-template-delete") False
