...
instance View NewView where
    html NewView { .. } = [hsx|
        ...
        <form method="POST" action={CreateEscalationPolicyAction} data-testid="escalation-policy-form" class="maxw-700">
            ...
            <button type="submit" class="btn btn-primary" data-testid="policy-submit">Create</button>
        </form>
    |]

-- Step list editor: fixed slots, empty rows are dropped server-side. Shared
-- with Edit (prefilled from the stored steps).
stepEditor :: [Team] -> [User] -> [EscalationStep] -> Html
stepEditor teams users steps = [hsx|
    <div class="mb-3" data-testid="policy-steps">
        <label class="form-label">Steps (after → target, optional unless-status)</label>
        ...
        stepRow (index, maybeStep) =
            ...
            in [hsx|
                <div class="input-group input-group-sm mb-1" data-testid={"policy-step-" <> show index}>
                    <span class="input-group-text">after (s)</span>
                    ...
                    <select name="stepUnless" class="form-select" data-testid="step-unless">
                        <option value="" selected={unlessValue == ""}>unless: —</option>
                        <option value="ack" selected={unlessValue == "ack"}>unless ack</option>
                    </select>
                </div>
            |]
        teamOption selected team = [hsx|
            <option value={"team:" <> tshow (get #id team)} selected={selected == "team:" <> tshow (get #id team)}>team: {team.name}</option>
        |]
        userOption selected user = [hsx|
            <option value={"user:" <> tshow (get #id user)} selected={selected == "user:" <> tshow (get #id user)}>user: {user.email}</option>
        |]
