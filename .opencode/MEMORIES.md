...
## Jobs
- Poll loops (PollZabbix/PollGrafana) STOP rescheduling when no enabled sources of their type exist; re-arm via `ensurePollerForSourceType` (Application/Service/PollerControl.hs) — called from SourcesController create/update/toggle + Provision.upsertSource. Sources inserted via raw SQL (seeds) need EnqueuePollers afterwards. Single-loop invariant: perform refuses to run when an OLDER-created sibling is running (EnqueuePollers inserts unconditionally per deploy; concurrent loops double-ingest → duplicate alert rows with identical started_at), and reschedule is a guarded INSERT ... WHERE NOT EXISTS pending — a duplicate loop's reschedule no-ops and it dies instead of deleting the survivor's successor.
...
## Facets & dashboards
- Facet refs: `field:` = EFFECTIVE field value (see below), `label:` = alerts.labels, `attr:` = materialized alerts.facets. Card match ops =/!=/~/in; `~` → LIKE via globToLike; `!=` never matches absent values. The prefixed refs exist only in dashboard card JSON — field_mappings.key is UNPREFIXED, interpreted by kind (field = closed enum env/host/service/check/severity/status, label = labels key, attr = Assets attribute name).
- **Effective field override (v1.28)**: a materialized facet named exactly `env`/`host`/`service` overrides the raw column EVERYWHERE — alert card (raw shown parenthesized), list tables, /alerts + env-page + API filters, overview env cards, grouping/notification match + {env} templates, WS payloads, audit export, metrics, LLM prompt, push. check/severity/status are NEVER overridable; blackouts + inventory ids (environment_id/host_id/service_id) stay raw (matched at ingest, pre-facets). Core: `effectiveFieldText`/`effectiveFieldSql`/`fieldOverridable` in Application.Pipeline.Grouping. SQL sites inline `coalesce(nullif(a.facets ->> 'env', ''), a.env)`; in QueryBuilder (append-only filterWhereSql) splice it behind `alerts.facets IS NOT NULL AND` (facets is NOT NULL by schema). Overview env cards group by effective env NAME (EnvCard.cardEnvName), inventory row attached only on name match; groups upsert their environment row from the effective env (Groups.effectiveEnvironmentRef — local copy, Ingest would cycle).
...
- Schema: alerts.facets jsonb + GIN (jsonb_path_ops), field_mappings (UNIQUE facet+rank), facet_backfill_jobs (cursor-chained, chunk 500). Facets materialize onto alerts.facets only — raw columns are never rewritten. Backfill needs the worker; attr facets need linked assets (EnrichAlertJob ran). Closed alerts keep stale facets (backfill covers non-closed only) — overrides on closed rows surface in audit export.
...
- Resolved-state reconcile (PollZabbix.reconcileProblemStates): one batched `trigger.get` per due cycle resolves local firing/ack alerts whose trigger's CURRENT value is OK — keyed on the TRIGGER id from the fingerprint, NOT alerts.external_id (refires don't rotate external_id, so the stored event id can point at an already-resolved older problem). Use trigger.get, NOT problem.get: problem.get has no `triggerids` param (objectids+object=0), and its `recent=false` returns only UNRESOLVED problems while recent=true covers just ok_period — an old resolve looks identical to a purge there. resolved_at is back-dated to the trigger's `lastchange` via a guarded UPDATE. The resolve targets the tracked row BY ID via Ingest.transitionAlert (extracted from ingest) — fingerprint rediscovery let duplicate non-closed rows eat the transition as an illegal no-op → endless "resolving" loop. Config keys (defaults): reconcileResolved(true), reconcileGraceSeconds(60, guards ingest/reconcile race via last_seen_at), reconcileIntervalSeconds(0=every cycle, tracked in sources.last_reconcile_at), absentResolveMinAgeSeconds(86400, trigger MISSING from trigger.get = deleted/permission-invisible → resolve only if alert older), eventPageLimit(1000). event.get is PAGED until a short page (post-outage backlog > 1 page used to skip OK events past the page boundary — the stuck-firing root cause).
