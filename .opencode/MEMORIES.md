...
- v1.18.0 card templates: `"forEach": "<facetRef>"` expands a card per distinct facet value (pin = appended Eq clause; title `{value}` placeholder, default = value); `"hideWhen": {"match": [...], "maxCount": N}` hides the card when count(match ++ hideWhen.match, non-closed) <= N. Expansion+hide eval live in expandDashboardCards (Application/Service/DashboardCards) — called by BOTH the Show controller and the Live broadcaster (templates never persist expanded). Expanded domId = dashboard-card-<index>-<value> (legacy env keys preserved); hidden cards render as empty `d-none` sections so WS "replaceOrPrepend" (parent #dashboard-cards) toggles them without DOM add/remove. Stale limitation: a forEach card whose last alert closes is only removed on page reload (closed-alert events skip re-render by design).
...
- FIXED (1.18.0) seed-halemans re-run dying on llm_agent_roles_default_idx: the default-enricher insert guarded on name only while the unique index is on is_default; integration leftovers (itest-default-role-*) hold is_default. Guard now also checks `NOT EXISTS (is_default)` in BOTH seed scripts. Symptom of the crash-loop: seed.done removed, worker never starts ("seed did not complete in time") — touch .devenv/state/halemans/seed.done manually when the DB is already seeded.
