...
## Versioning
- ANY code change bumps the version per semver (patch: fixes/internal, minor: features/compatible, major: breaking). Version lives in Halemans.cabal (`version:` field); releases are git tags `vX.Y.Z` pushed to both remotes (origin=gitea, github). When several version bumps accumulate in one uncommitted session, commit them together but tag ONLY the latest version.
