...
spec = describe "Application.Helper.DashboardConfig" do
        ...
        let v2json =
                Aeson.toJSON
                    [ object
                        ...
                        , "match"
                            .= [ object ["facet" .= ("attr:Service" :: Text), "op" .= ("=" :: Text), "value" .= ("PostgreSQL" :: Text)]
    ...
    describe "forEach + hideWhen (card templates)" do
        ...
        it "decodes forEach and hideWhen" do
            decodeDashboardConfig templateJson
                `shouldBe` Right
                    [ DashboardCard
...
legacyCard env statuses severities =
    DashboardCard
        { cardTitle = Nothing
        , cardMatch =
            [MatchClause (FacetField FieldEnv) OpEq env []]
