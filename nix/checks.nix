...
in
{
    ...
    "integration-tests" = lib.mkForce (pkgs.stdenv.mkDerivation {
        ...
        MOCK_ZABBIX_PY = "${self}/nix/mocks/mock_zabbix.py";
        buildPhase = ''
            export IHP_LIB=${ihpLib}
            ...
            export ZABBIX_TOKEN="test-zabbix-token"
            ...
            python3 "$MOCK_ZABBIX_PY" > "$TMPDIR/mock-zabbix.log" 2>&1 &
            mocks_up=0
            for i in $(seq 1 30); do
                if python3 -c "
import urllib.request
for url in ('$HALEMANS_CONFLUENCE_URL/health', '$HALEMANS_JIRA_URL/health', '$LLM_ENDPOINT/health', 'http://127.0.0.1:18085/health', '$MOCK_GRAFANA_URL/health', '$MOCK_ZABBIX_URL/health'):
...
"; then mocks_up=1; break; fi
            ...
            if [ "$mocks_up" != 1 ]; then
                ...
                tail -n 20 "$TMPDIR/mock-confluence.log" "$TMPDIR/mock-jira.log" "$TMPDIR/mock-llm.log" "$TMPDIR/mock-assets.log" "$TMPDIR/mock-grafana.log" "$TMPDIR/mock-zabbix.log" >&2
                exit 1
            fi
